...
const fieldInput =
  "!h-10 rounded-[0.55rem] border border-(--border-subtle) bg-(--bg-inset) text-(--text-primary) placeholder:text-(--text-disabled) focus-visible:border-(--accent-primary) focus-visible:ring-1 focus-visible:ring-(--accent-primary) transition-colors text-sm";
            ...
            return (
              <button
                ...
                className={`flex items-center justify-between rounded-[0.75rem] border px-4 py-3.5 text-left transition-all duration-150 ${
                  isSelected
                    ? "border-(--accent-primary)/40 bg-[rgba(122,213,221,0.08)]"
                    : "border-(--border-subtle) bg-(--bg-inset) hover:border-(--accent-primary)/25 hover:bg-(--bg-elevated)"
                }`}
              >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between border-b border-white/5 px-4 py-3">
                      ...
                      {totalBlocks > 0 && (
                        <span className="rounded-full bg-(--accent-primary)/12 px-2.5 py-0.5 text-[0.58rem] font-semibold uppercase tracking-[0.12em] text-(--accent-primary)">
                      ...
                      {blocks.length > 0 && (
                        <div className="space-y-1.5">
                          ...
                          {blocks.map((block, idx) => (
                            <div
                              key={`${staff.id}-${idx}`}
                              className="flex items-center justify-between rounded-[0.5rem] border border-(--accent-primary)/20 bg-[rgba(122,213,221,0.05)] px-3 py-2"
                            >
                              <span className="text-xs text-(--text-secondary)">
                                {
                                  DAYS_OF_WEEK.find(
